    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                ...
                bat 'for /f "tokens=5" %a in (\'netstat -aon ^| findstr :5500\') do taskkill /F /PID %a >nul 2>&1 || echo No process running on port 5500'
                bat 'start /b cmd /c "node server.js > server.log 2>&1"'
